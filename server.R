...
shinyServer(function(input, output) {
  ...
  observe({
    input$Calc
    values$int <- isolate({
      input$Housing_loan_amt * input$int_rate *
        recode(input$durtn, "1 = '1'; 2 = '0.25'; 
               3 = '0.08333333'")/100 * input$time_period  
    })
    values$amt <- ({isolate(input$Housing_loan_amt) + values$int
    })
  ...
  output$text_Loan_amt <- renderText({
    input$Calc
    paste("Loan amount: [$]", isolate(input$Housing_loan_amt))
  })
  ...
  output$text_amtplus <- renderText({
    if(input$Calc == 0) ""
      ...
      paste("Total Housing Loan Amount, with Interest [$]:", values$amt)
  })
